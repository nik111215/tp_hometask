...
TEST(TreeTestCase, PathNotFound) {
    ASSERT_THROW(GetTree("/bla_bla_blAA", false), std::invalid_argument);
}

TEST(TreeTestCase, IsDirectory) {
    std::string dir_name_ = __FILE__;
    ASSERT_THROW(GetTree(dir_name_, false), std::invalid_argument);
}
...
TEST(TreeTestCase, FileNodeName) {
    ...
    FileNode f_node_ = GetTree(curr_path_.string(), true);
...
TEST(TreeTestCase, FileNodeChildren) {
    ...
    FileNode f_node_ = GetTree(curr_path_.string(), false);
...
TEST(TreeTestCase, ComparisonOperatorEquality) {
    ...
    FileNode f_node_ = GetTree(curr_path_.c_str(), true);
...
TEST(TreeTestCase, ComparisonOperatorInequality) {
    auto curr_path_ = boost::filesystem::current_path();
    FileNode f_node_1 = GetTree(curr_path_.c_str(), true);
    curr_path_ += "/..";
    FileNode f_node_2 = GetTree(curr_path_.c_str(), true);
